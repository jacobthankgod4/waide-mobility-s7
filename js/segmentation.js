    ...
    updateUseCases() {
        const useCases = {
            ...
            general: [
                ...
                { title: "See Through Walls Like Superman", desc: "Audio radar reveals obstacles, exits, and safe paths before you encounter them" }
            ]
        };
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.waideSegmentation.trackBehavior();
        ...
        const useCaseContainer = document.querySelector('.use-cases, .features');
        if (useCaseContainer && useCases[this.segment]) {
            const segmentUseCases = useCases[this.segment];
            const useCaseElements = useCaseContainer.querySelectorAll('.use-case, .feature-item');
            
            segmentUseCases.forEach((useCase, index) => {
                if (useCaseElements[index]) {
                    const titleElement = useCaseElements[index].querySelector('h3, .title');
                    const descElement = useCaseElements[index].querySelector('p, .description');
                    
                    if (titleElement) titleElement.textContent = useCase.title;
                    if (descElement) descElement.textContent = useCase.desc;
                }
            });
        }
    }
    ...
    updateTestimonials() {
        const testimonials = {
            healthcare: [
                { name: "Dr. Sarah Okafor", role: "Chief Medical Officer, Lagos General Hospital", quote: "Since implementing Waide navigation, our emergency response time improved by 67%. We've literally saved lives by helping patients find critical care faster. This should be mandatory in every hospital." },
                { name: "James Adebayo", role: "Hospital Administrator, Abuja Medical Center", quote: "Our patient satisfaction scores jumped 40% and staff productivity increased dramatically. Waide pays for itself through operational efficiency alone." }
            ],
            retail: [
                { name: "Amina Hassan", role: "Mall Director, Victoria Island Shopping Center", quote: "We captured $2M in additional revenue from previously underserved customers. Waide transformed our mall into Lagos's most accessible shopping destination." },
                { name: "David Chen", role: "Retail Operations Manager", quote: "Customer complaints dropped 85% and our accessibility reputation attracts shoppers from across the city. Best investment we've made." }
            ],
            hospitality: [
                { name: "Grace Okonkwo", role: "Hotel General Manager, Transcorp Hilton", quote: "Guest satisfaction scores increased 89% and we're getting incredible reviews about accessibility. Waide turned our property into a destination that guests actively recommend." },
                { name: "Michael Thompson", role: "Resort Director", quote: "The viral marketing value alone is worth millions. Guests can't stop posting about how easy it is to navigate our resort." }
            ],
            transportation: [
                { name: "Captain Folake Adesanya", role: "Airport Operations Director, Murtala Muhammed", quote: "Passenger complaints dropped 70% and our accessibility compliance is now industry-leading. Waide eliminated the chaos that cost us millions in delays." },
                { name: "Robert Okwu", role: "Terminal Manager", quote: "We're capturing the accessible travel market that other airports miss. Revenue from this segment increased 300% since implementation." }
            ],
            education: [
                { name: "Prof. Kemi Rotimi", role: "Vice-Chancellor, University of Lagos", quote: "Student dropout rates among disabled students fell to zero. Waide ensures every student can access education with dignity and independence." },
                { name: "Dr. Emmanuel Nwosu", role: "Student Affairs Director", quote: "We're now the most accessible university in West Africa. Applications from disabled students increased 250% because of our inclusive reputation." }
            ],
            government: [
                { name: "Hon. Fatima Bello", role: "Commissioner for Public Works", quote: "We avoided potential lawsuits worth millions and now serve every citizen with equal dignity. Waide protects both our budget and our values." },
                { name: "Architect Tunde Bakare", role: "Public Buildings Director", quote: "Compliance is guaranteed and citizen satisfaction with public services improved dramatically. This is how government should serve everyone." }
            ],
            ngo: [
                { name: "Dr. Aisha Mahmoud", role: "Country Director, UNICEF Nigeria", quote: "Waide helped us reach 50,000 previously underserved children. During the last flood emergency, our navigation system guided families to safety and saved lives." },
                { name: "Rev. Peter Okafor", role: "Program Director, Red Cross", quote: "We achieved 67% reduction in service inequality. Every vulnerable person can now access our programs with dignity and independence." }
            ],
            professional: [
                { name: "Adaora Obi", role: "Facility Manager, Corporate Towers", quote: "We captured $500K in revenue from accessible events and conferences. Competitors lose this business because they can't serve disabled professionals." },
                { name: "Samuel Adeyemi", role: "Property Developer", quote: "Waide pays for itself in 90 days through increased occupancy and premium pricing. Accessibility is now our competitive advantage." }
            ],
            disability: [
                { name: "Blessing Okoro", role: "Wheelchair User, Lagos", quote: "For the first time in my life, I can explore any building independently. Waide gave me back my dignity and freedom. I finally feel truly included in society." },
                { name: "Ibrahim Musa", role: "Visually Impaired Professional", quote: "I navigate confidently without asking for help. The multilingual support in Hausa makes me feel at home everywhere I go." }
            ],
            general: [
                { name: "Chief Emeka Okafor", role: "Community Leader, Abuja", quote: "During the last flood, Waide navigation helped our entire community reach safety. This technology saves lives and builds resilient communities." },
                { name: "Dr. Zainab Ahmed", role: "Climate Resilience Expert", quote: "Waide is essential infrastructure for climate adaptation. Every African city needs this to protect vulnerable populations during emergencies." }
            ]
        };

        const testimonialContainer = document.querySelector('.testimonials');
        if (testimonialContainer && testimonials[this.segment]) {
            const segmentTestimonials = testimonials[this.segment];
            const testimonialElements = testimonialContainer.querySelectorAll('.testimonial');
            
            segmentTestimonials.forEach((testimonial, index) => {
                if (testimonialElements[index]) {
                    const quoteElement = testimonialElements[index].querySelector('.quote, blockquote');
                    const nameElement = testimonialElements[index].querySelector('.name, .author');
                    const roleElement = testimonialElements[index].querySelector('.role, .title');
    ...
    updateCTA() {
        const ctaButtons = document.querySelectorAll('.cta-button:not(.hero .cta-button)');
        const heroContent = this.getHeroContent();
        
        ctaButtons.forEach(button => {
            if (button) button.textContent = heroContent.cta;
        });
    }
