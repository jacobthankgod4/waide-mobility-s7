...
function updatePricing(isYearly) {
  ...
  pricingCards.forEach((card, index) => {
    ...
    if (isYearly && index < 2) { // Only for Starter and Professional
      const monthly = index === 0 ? 399 : 1199
      const savings = monthly * 2 // 2 months free
  ...
  // Show/hide limited time offer
  let limitedOffer = document.getElementById('limited-offer')
  if (isYearly) {
    if (!limitedOffer) {
      limitedOffer = document.createElement('div')
      limitedOffer.id = 'limited-offer'
      limitedOffer.style.cssText = `
        text-align: center;
        margin: 1rem 0;
        padding: 8px 16px;
        background: linear-gradient(45deg, #ff6b6b, #ee5a24);
        ...
        animation: pulse 2s infinite;
      `
      limitedOffer.innerHTML = '🔥 Limited Time: Get 2 months FREE on annual plans!'
      document.querySelector('.pricing-toggle').parentNode.insertBefore(limitedOffer, document.querySelector('.pricing-grid'))
    }
    limitedOffer.style.display = 'block'
...
function initializeNavigation() {
  ...
  if (mobileDropdownToggle && mobileDropdownContent) {
    mobileDropdownToggle.addEventListener('click', () => {
      ...
      const arrow = mobileDropdownToggle.querySelector('span')
      if (arrow) {
        arrow.textContent = mobileDropdownContent.classList.contains('active') ? '▲' : '▼'
      }
    })
  }
